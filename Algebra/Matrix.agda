...
scalar-distributivity : ∀ {n : Nat} {{SR : Rng A}} (x y : A) (v : [ A ^ n ]) → scaleV (x + y) v ≡ addv (scaleV x v) (scaleV y v)
...
scalar-distributivity2 : ∀ {n} {{SR : Rng A}} (s : A) (x y : [ A ^ n ]) → scaleV s (addv x y) ≡ addv (scaleV s x) (scaleV s y)
